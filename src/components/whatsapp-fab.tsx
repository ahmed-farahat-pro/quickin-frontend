...
export default function WhatsAppFab() {
  ...
  return (
    <>
      {/* `bottom` lives in the stylesheet (not inline) so the phone media query can
          win: on ≤640px the fixed "download the app" bar (~64px + safe area,
          zIndex 950) owns the bottom edge, so the FAB is raised clear of it. */}
      <style>{`
        .qk-fab-wa { bottom: 22px; }
        @media (max-width: 640px) {
          .qk-fab-wa { bottom: calc(80px + env(safe-area-inset-bottom)); }
        }
      `}</style>
      <a
        href={href}
        ...
        style={{
          position: 'fixed',
          // Inline-START: bottom-left in LTR (mirrors to the right in RTL), leaving
          // the opposite corner free for the burgundy "Add listing" FAB.
          insetInlineStart: 22,
          zIndex: 900,
          width: 56,
          height: 56,
